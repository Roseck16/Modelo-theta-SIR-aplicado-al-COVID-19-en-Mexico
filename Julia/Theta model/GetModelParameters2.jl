...
struct StaticParams{M<:Real}
    ...
    function StaticParams(opt::Dict{String, Any}, saved::Dict{String, Real})
        t0 = get_data("t0", opt, saved)
        tMAX = get_data("tMAX", opt, saved)
        t_iCFR = get_data("t_iCFR", opt, saved)
        t_θ0 = get_data("t_θ0", opt, saved)
        t_n = get_data("t_n", opt, saved)
        new{Real}(t0, tMAX, t_iCFR, t_θ0, t_n)
    end
end
...
struct γs{M<:Float64}
    ...
    function γs(opt::Dict{String, Any}, saved::Dict{String, Real})
        ...
        new{Float64}(γ_d, γ_E, γ_I, γ_Iu, γ_IDu, γ_Hr, γ_Hd)
    end
end
...
function get_ð(t::Integer, data::Data, stpms::StaticParams, gammas::γs)
    ...
    elseif t > t_n
        ...
        for r in 1:q
            index2 = t - r + delay
            den2 = index2 >= q ? infec[q-r] : index2 <= 1 ? infec[1] : infec[index2]
            if den1 - den2 != 0
                num2 = index2 >= q ? infec_medic[q-r] : index2 <= 1 ? infec_medic[1] : infec_medic[index2]
...
function get_iCFR(t::Integer, data::Data, stpms::StaticParams, gammas::γs)
    ...
    elseif t > t_iCFR
        ...
        for r in 1:q
            ...
            if den != 0
                num2 = index2 >= q ? dead[q] : index2 <= 1 ? dead[1] : dead[index2]
...
function TimeParams(
    ...
    return [ωs[1], ωs[2], θs[1], θs[2], values[1], values[2], values[3], 0, data.exported[t]]
    # [ω_0, ω, θ_0, θ, ω_u0, η, ρ, τ1, τ2]
end
#end
...
function βs(t::Integer, opt::Dict{String, Any}, saved::Dict{String, Real}, tmpms::Vector{Float64}, ms::Msλs, gammas::γs)
    ...
    β_e, β_I, β_Iu, β_IDu = (β_e0, β_I0, β_Iu0, β_IDu0) .* ms

    num = tmpms[6] * (β_e/gammas.γ_E + β_I/gammas.γ_I + (1 - θ - tmpms[5]) * β_Iu/gammas.γ_Iu + tmpms[5])

    den = (1 - tmpms[6]) * (tmpms[7] * (θ - tmpms[2]) * (1/gammas.γ_Hr) + tmpms[2] * (1/gammas.γ_Hd))

    β_hr = num / den

    return [β_e, β_I, β_Iu, β_IDu, β_hr, β_hr]
end
#end
